            ...
            <div className="flex gap-4 flex-wrap">
                {terms.map(term => (
                    <div key={term.id} className="flex gap-16 group/term p-2 border border-stone-400">
                        <div className="">
                            <input type="text" className="font-semibold text-stone-800 mb-2" onBlur={(e) => timeActions.updateTerm(term.id, { name: e.target.value })} defaultValue={term.name} />
                        ...
                        <div className="flex gap-2 group-hover/term:opacity-100 opacity-0 transition-opacity duration-300">
                            <button className="text-stone-600" onClick={() => timeActions.removeTerm(term.id)}>
                                <Trash className="w-4 h-4" />
                            </button>
                        </div>
                    </div>
                ))}
                <button className="min-w-40 min-h-16 flex items-center justify-center text-stone-600 cursor-pointer bg-primary text-white hover:bg-primary/80 transition-colors duration-300" onClick={() => timeActions.addTerm({
                    name: 'תקופה חדשה',
