            ...
            >
                ...
                >
                    ...
                    <div className="flex-1 bg-white w-2xl border border-stone-300 rounded-md">
                        {content === 'new' && <JournalNew isOpen={isOpen} setIsOpen={setIsOpen} />}
                        {content === 'content' && <Journal isOpen={isOpen} setIsOpen={setIsOpen} />}
                    </div>
                </div>
            </div>
        </>
...
const JournalTabDiv = tw.button`min-w-32 h-[40px] flex items-center justify-center gap-3 cursor-pointer relative overflow-hidden flex-shrink-0 transition-all duration-300
    ${props => props.$active ? 
        ...
        'px-8 bg-stone-100 group-hover:bg-stone-200 border-x border-t border-stone-200 text-stone-700 hover:text-stone-800 z-1'}
    `
const JournalTabHandle = tw.div`absolute top-1 left-1/2 -translate-x-1/2 h-1 rounded-full transition-all duration-300
    ${props => props.$active ? 'bg-stone-200/30 w-12 group-hover:w-16' : 'bg-stone-600/30 w-3 group-hover:w-6'}`

function JournalTab({ onClick, label, Icon, active}) {
    return (
        <JournalTabDiv $active={active} onClick={onClick}
            style={{
        ...
        >
            <JournalTabHandle $active={active} />
