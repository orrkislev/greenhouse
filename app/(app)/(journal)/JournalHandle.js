...
export default function JournalHandle() {
    ...
    return (
        <>
            ...
            <div
                className={`fixed max-h-[70vh] md:max-h-[70vh] left-1/2 -translate-x-1/2 w-full md:max-w-2xl transition-all duration-700 z-50 ${isOpen
                    ? "bottom-0 md:bottom-[50%] md:translate-y-[20vh] ease-[cubic-bezier(0.34,1.56,0.64,1)]"
                    : isHovered
...
function JournalTab({ onClick, label, Icon, notifyNewLog, isActive = false }) {
    return (
        ...
        >
            ...
            {notifyNewLog && <div className="w-3 h-3 md:w-4 md:h-4 bg-red-500 rounded-full animate-pulse" />}
        </JournalTabDiv>
    )
}