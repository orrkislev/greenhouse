...
export default function MainSchedule() {
    ...
    const allTasks = [...personalTasks, ...assignedTasks, ...projectTasks];
    const todayTasks = allTasks
        .filter(t => t.planned_date === todayStr && t.status !== 'archived' && t.status !== 'closed')
        .map(t => ({ ...t, _itemType: 'task' }));

    // Merged list: events first (sorted by time), then tasks
    const merged = [...allEvents, ...todayTasks];

    const nextEvent = allEvents.find(event => {
        let eventDate = new Date(event.date || today);
    ...
    return (
        <Box2 label="מה יש לי היום" className="col-start-1 row-start-1 row-span-4 flex-1 relative" LabelIcon={Calendar}>
            ...
            <div className="flex flex-col gap-2 mt-2">
                {!isEmpty ? merged.map(item => {
                    if (item._itemType === 'task') {
                        return <TaskRow key={`task-${item.id}`} task={item} />;
                    }
                    ...
                    <div className="flex flex-col items-center justify-center h-full">
                        ...
                        <div className="text-xs text-muted-foreground">אין אירועים ומשימות היום</div>
                    </div>
                )}
            </div>
...
function TaskRow({ task }) {
    ...
    return (
        <div className={`flex items-center gap-2 rounded p-1 text-xs border border-border bg-background
            ${isDone ? 'opacity-50' : 'hover:bg-muted'}`}>
            <CheckSquare2 className={`w-3.5 h-3.5 shrink-0 ${isDone ? 'text-green-500' : 'text-muted-foreground'}`} />
            <span className={`flex-1 truncate ${isDone ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
                {task.title}
            </span>
            {task.group_id && (
                <span className="text-[10px] text-orange-500 shrink-0">מוקצה</span>
            )}
        </div>
    );
}
