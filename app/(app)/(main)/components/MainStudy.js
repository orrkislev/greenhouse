                    ...
                    <Link href={`/study/?id=${path.id}`} key={path.id}>
                        <div className="group/path cursor-pointer">
                            <div className="w-full aspect-[7/3] relative" 
                                style={{ backgroundImage: path.metadata?.image ? `url(${path.metadata?.image})` : 'linear-gradient(to right, #f7797d, #FBD786, #C6FFDD)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
                                ...
                                <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-transparent group-hover/path:bg-stone-900/30 transition-all duration-200">
                                    <h1 className="font-bold p-2 bg-white group-hover/path:text-stone-900 group-hover/path:scale-105 transition-all duration-200">{path.title}</h1>
                                </div>
                            </div>
                        </div>
                    </Link>
