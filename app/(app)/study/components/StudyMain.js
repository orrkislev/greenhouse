    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex gap-4 flex-wrap">
                {paths.map(path => (
                    <Link key={path.id} className="bg-white border border-stone-200 p-4 rounded-md min-w-64 min-h-64 relative cursor-pointer hover:bg-stone-100 hover:border-stone-300 transition-all duration-300" href={`/study?id=${path.id}`}>
                        ...
                        <div className="text-2xl font-bold mt-2">{path.title}</div>
